import { firstText, getPathValue as path, getProperty as property, getRecordProperty as recordProperty, isRecord } from "../sharedTypes.js";
...
export function resolveSokosumiIdentity(value, options = {}) {
    ...
    const id = firstText(...HEADER_USER_ID_KEYS.map((key) => headers[key]), ...ID_KEYS.map((key) => property(root, key)), ...ID_KEYS.map((key) => property(metadata, key)), path(root, "sokosumi", "userId"), path(root, "sokosumi", "user", "id"), path(metadata, "sokosumi", "userId"), path(metadata, "sokosumi", "user", "id"), path(root, "user", "id"), path(metadata, "user", "id"), path(root, "actor", "id"), path(metadata, "actor", "id"), path(root, "customer", "id"), path(metadata, "customer", "id"), path(root, "owner", "id"), path(metadata, "owner", "id"), path(root, "createdBy", "id"), path(metadata, "createdBy", "id"), path(root, "requester", "id"), path(metadata, "requester", "id"), path(root, "task", "userId"), path(root, "task", "user", "id"), path(metadata, "task", "userId"), path(metadata, "task", "user", "id"), path(root, "event", "userId"), path(root, "event", "user", "id"), path(metadata, "event", "userId"), path(metadata, "event", "user", "id"), ...candidates.flatMap((candidate) => ID_KEYS.map((key) => property(candidate, key))));
    if (!id)
        return null;
    const userObject = candidates.find((candidate) => firstText(property(candidate, "id"), property(candidate, "userId"), property(candidate, "sokosumiUserId"), property(candidate, "sub")) === id) || recordProperty(root, "user") || recordProperty(rootSokosumi, "user");
    return {
        id,
        name: firstText(property(userObject, "name"), path(root, "user", "name"), path(metadata, "user", "name"), path(root, "sokosumi", "user", "name"), path(metadata, "sokosumi", "user", "name")) ?? "",
        image: firstText(property(userObject, "image"), property(userObject, "avatarUrl"), path(root, "user", "image"), path(metadata, "user", "image"), path(metadata, "user", "avatarUrl"), path(root, "sokosumi", "user", "image"), path(metadata, "sokosumi", "user", "image")) ?? "",
        organizationId: firstText(...HEADER_ORGANIZATION_ID_KEYS.map((key) => headers[key]), ...candidates.flatMap((candidate) => [
        ...
        organizationSlug: firstText(...HEADER_ORGANIZATION_SLUG_KEYS.map((key) => headers[key]), ...candidates.flatMap((candidate) => [
        ...
        workspaceId: firstText(...HEADER_WORKSPACE_ID_KEYS.map((key) => headers[key]), ...candidates.flatMap((candidate) => [
        ...
        source: firstText(property(root, "protocol"), property(metadata, "protocol"), property(root, "source"), property(metadata, "source"), property(root, "origin"), property(metadata, "origin"), headers["x-delegation-user-id"] ? "sokosumi_delegation_headers" : "", headers["x-sokosumi-user-id"] ? "sokosumi_headers" : "") ?? ""
    };
}
