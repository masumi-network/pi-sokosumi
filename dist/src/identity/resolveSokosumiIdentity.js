...
export function resolveSokosumiIdentity(value, options = {}) {
    ...
    const id = firstString(...HEADER_USER_ID_KEYS.map((key) => headers[key]), ...ID_KEYS.map((key) => root[key]), ...ID_KEYS.map((key) => metadata[key]), root.sokosumi?.userId, root.sokosumi?.user?.id, metadata.sokosumi?.userId, metadata.sokosumi?.user?.id, root.user?.id, metadata.user?.id, root.actor?.id, metadata.actor?.id, root.customer?.id, metadata.customer?.id, root.owner?.id, metadata.owner?.id, root.createdBy?.id, metadata.createdBy?.id, root.requester?.id, metadata.requester?.id, root.task?.userId, root.task?.user?.id, metadata.task?.userId, metadata.task?.user?.id, root.event?.userId, root.event?.user?.id, metadata.event?.userId, metadata.event?.user?.id, ...candidates.flatMap((candidate) => ID_KEYS.map((key) => candidate[key])));
    if (!id)
        return null;
    ...
    return {
        ...
        image: firstString(userObject?.image, userObject?.avatarUrl, root.user?.image, metadata.user?.image, metadata.user?.avatarUrl, root.sokosumi?.user?.image, metadata.sokosumi?.user?.image),
        ...
        ]), root.organizationId, metadata.organizationId, userObject?.organizationId, root.user?.organizationId, metadata.user?.organizationId, root.organization?.id, metadata.organization?.id, root.workspace?.organizationId, metadata.workspace?.organizationId),
        ...
        ]), root.organizationSlug, metadata.organizationSlug, userObject?.organizationSlug, root.user?.organizationSlug, metadata.user?.organizationSlug, root.organization?.slug, metadata.organization?.slug, root.workspace?.organizationSlug, metadata.workspace?.organizationSlug),
        ...
        ]), root.workspaceId, metadata.workspaceId, userObject?.workspaceId, root.user?.workspaceId, metadata.user?.workspaceId, root.workspace?.id, metadata.workspace?.id),
        source: firstString(root.protocol, metadata.protocol, root.source, metadata.source, root.origin, metadata.origin, headers["x-delegation-user-id"] ? "sokosumi_delegation_headers" : "", headers["x-sokosumi-user-id"] ? "sokosumi_headers" : "")
    };
}
