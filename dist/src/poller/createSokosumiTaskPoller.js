...
export function createSokosumiTaskPoller({ client, intervalMs = 15000, limit = 20, maxPages = 10, logger = console, shouldProcessEvent = defaultShouldProcessEvent, hasTaskProgress = defaultHasTaskProgress, createReopenedEvent = defaultCreateReopenedEvent, createRunningEvent = defaultCreateRunningEvent, createCanceledEvent = defaultCreateCanceledEvent, createCompletedEvent, createFailedEvent = defaultCreateFailedEvent, createStaleInputRequiredEvent, inputRequiredTimeoutMs = 0, now = () => new Date(), beforeTaskEventCreated, afterTaskEventCreated }) {
    ...
    async function scanEventPages() {
        ...
        if (headPageBudget > 0) {
            ...
            if (!backlogCursor)
                backlogCursor = headScan.nextCursor;
        }
        if (resumeCursor) {
        ...
        try {
            ...
            if (restartInput) {
                ...
                if (reopenedEvent) {
                    ...
                    if (createdReopenedEvent)
                        activeTask = appendCreatedTaskEvent(activeTask, event.taskId, createdReopenedEvent, reopenedEvent);
                }
            }
            if (!commentOnly) {
                const runningEvent = createRunningEvent ? await createRunningEvent({ event, task: activeTask }) : undefined;
                if (runningEvent)
                    await createTaskEventOrSkipInvalidStatus(event.taskId, runningEvent, event);
            }
            ...
            if (failedEvent)
                await createTaskEventWithCommentFallback(event.taskId, failedEvent, event, { allowCommentFallback: false });
            throw error;
        }
    }
        ...
        try {
            ...
            if (isInvalidStatusTransitionError(error) && body?.status) {
                ...
                if (recoveredTaskEvent)
                    return recoveredTaskEvent;
                log(logger, "sokosumi_task_status_transition_skipped", { eventId: triggerEvent?.id, taskId, status: body.status, reason: "invalid_status_transition" });
                return undefined;
            }
            throw error;
        }
    ...
    async function createTaskEventWithCommentFallback(taskId, body, triggerEvent, { allowCommentFallback = true } = {}) {
        try {
            ...
            if (allowCommentFallback && isInvalidStatusTransitionError(error) && body?.status && body?.comment) {
                log(logger, "sokosumi_task_status_transition_fallback", { eventId: triggerEvent?.id, taskId, status: body.status, reason: "invalid_status_transition" });
    ...
    async function recoverInvalidStatusTransition(taskId, body, triggerEvent, { allowCommentFallback = true } = {}) {
        if (!body?.status || typeof client.updateTask !== "function")
            return undefined;
        if (body.comment && !allowCommentFallback)
            return undefined;
        ...
        try {
            ...
            log(logger, "sokosumi_task_status_transition_recovered", { eventId: triggerEvent?.id, taskId, status: body.status, taskStatus });
            return {
                taskId,
                ...(commentEvent || {}),
                ...body,
            ...
            log(logger, "sokosumi_task_status_transition_recovery_failed", {
    ...
    async function handleCancelRequestedEvent(event, task) {
        ...
        if (canceledEvent)
            await client.createTaskEvent(event.taskId, canceledEvent);
    ...
    async function notifyAfterTaskEventCreated(input) {
        if (!afterTaskEventCreated)
            return;
        try {
            await afterTaskEventCreated(input);
        }
        catch (error) {
            log(logger, "sokosumi_task_after_event_hook_failed", {
    ...
    async function prepareTaskEventForCreate(input) {
        if (!beforeTaskEventCreated)
            return input.taskEvent;
        try {
            const nextTaskEvent = await beforeTaskEventCreated(input);
            return nextTaskEvent || input.taskEvent;
        }
        catch (error) {
            log(logger, "sokosumi_task_before_event_hook_failed", {
                eventId: input?.event?.id,
                taskId: input?.taskId,
                message: error.message
            }, "error");
...
function defaultShouldProcessEvent(event, task) {
    if (isTerminalTaskProgress(task?.status))
        return isRestartInputEvent(event, task);
    if (normalizeStatus(event.status) === SOKOSUMI_TASK_EVENT_STATUS.READY)
        return true;
    return isInputProvidedEvent(event, task);
}
function defaultHasTaskProgress(task, triggerEvent) {
    if (isTerminalTaskProgress(task?.status) && !isRestartInputEvent(triggerEvent, task))
        return true;
    ...
    return task.events.some((event, index) => {
        if (!isCoworkerEvent(event))
            return false;
        if (!isAfterTriggerEvent({ event, index, triggerEvent, triggerIndex }))
            return false;
        if (isCoworkerProgressStatus(event.status))
            return true;
...
function shouldCreateCommentOnlyEvent(task, triggerEvent) {
    if (isTerminalTaskProgress(task?.status))
        return !isRestartInputEvent(triggerEvent, task);
    if (!Array.isArray(task?.events))
        return false;
...
function shouldTreatCommentOnlyCoworkerRepliesAsProgress(task, triggerEvent) {
    if (isRestartInputEvent(triggerEvent, task))
        return true;
    if (!Array.isArray(task?.events))
        return false;
    const latestProgress = findLatestCoworkerProgressBeforeTrigger(task.events, triggerEvent);
...
function isCommentOnlyCoworkerProgressEvent(event) {
    if (event?.status !== null && event?.status !== undefined && normalizeStatus(event.status))
        return false;
...
function isRestartInputEvent(event, task) {
    ...
    if (event?.status !== null && event?.status !== undefined && status !== SOKOSUMI_TASK_EVENT_STATUS.READY)
        return false;
    if (!Array.isArray(task?.events))
        return false;
    const latestProgress = findLatestCoworkerProgressBeforeTrigger(task.events, event);
...
function findLatestCoworkerProgressBeforeTrigger(events, triggerEvent) {
    ...
    events.forEach((event, index) => {
        ...
        if (!isCoworkerProgressStatus(event.status))
            return;
        if (!isBeforeTriggerEvent({ event, index, triggerEvent, triggerIndex }))
            return;
...
function findStaleInputRequiredEvent(task, { timeoutMs, now }) {
    ...
    if (normalizeStatus(latestProgress?.status) !== SOKOSUMI_TASK_EVENT_STATUS.INPUT_REQUIRED)
        return null;
    if (hasUserPayloadAfterEvent(task.events, latestProgress))
        return null;
    const progressTime = getEventTimestamp(latestProgress);
    const nowTime = getEventTimestamp({ createdAt: now });
    if (progressTime === undefined || nowTime === undefined)
        return null;
    if (nowTime - progressTime < timeoutMs)
        return null;
    return latestProgress;
}
...
//# sourceMappingURL=createSokosumiTaskPoller.js.map